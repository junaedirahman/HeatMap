...
function epochToJsDate(epochTime){
  ...
  document.addEventListener("DOMContentLoaded", function () {
    const footerAfterLogin = document.querySelector('.footer-after-login');
    const isLoggedIn = false; // Pastikan ini sesuai dengan kondisi Anda

    if (isLoggedIn) {
        footerAfterLogin.classList.add('active');
    }
});
  ...
  // function to export table to excel (.xls)  
  ...
  const setupUI = (user) => {
    if (user) {
      ...
      downloadButtonElement.addEventListener('click', (e) => {
        exportTableToExcel('readings-table', 'Data Suhu & RH'); //KOREKSI/////////
      });
